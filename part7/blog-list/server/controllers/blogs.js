...
blogRouter.post(
  ...
  async (request, response) => {
    ...
    if (!url || typeof url !== 'string') error.error += 'url must be string.'

    if (error.error !== '') return response.status(400).json(error)
...
blogRouter.delete('/:id', extractTokenPayload, async (request, response) => {
  ...
  if (!blog) return response.status(404).json({ error: 'No blog available' })
...
blogRouter.put(
  ...
  async (request, response) => {
    ...
    if (url && typeof url !== 'string') errors.push('url must be a string')
    ...
    if (!blog)
      return response
        .status(404)
        .json({ error: 'No blog available with given id' })
